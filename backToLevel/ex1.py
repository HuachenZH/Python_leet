...
class Solution:
    ...
    def decode_cesar(self, texte:str) -> int:
        ...
        valueDict={}
        scorelist=[]
        # sorted by a to z. Made in Excel
        ...
        for i in range(1,26):
            valueDict=self.value(i, texte) # score of a decoding
            vectScoped=list([valueDict[key] for key in sorted(valueDict)])
            vectScoped=numpy.array(vectScoped)
            score=(numpy.sum((vectStandard[:]-vectScoped[:])**2))**0.5
            scorelist.append(score)
            # print(score)
        print('min score is : '+str(min(scorelist)))
        scorelist=numpy.array(scorelist)
        print('decoded messaged :')
        print('value of shift : '+ str(numpy.where(scorelist == scorelist.min())[0][0]+1))
        return self.shift(numpy.where(scorelist == scorelist.min())[0][0]+1,texte)
